...
namespace VetFlow.Server.Controllers.auth
{
    ...
    public class ForgotPasswordController(IGraphService graphService, ILogger<ForgotPasswordController> logger, IMailerSendService mailerSend) : ControllerBase
    {
        private readonly IGraphService _graphService = graphService;
        private readonly ILogger<ForgotPasswordController> _logger = logger;
        private readonly IMailerSendService _mailerSend = mailerSend;
        ...
        public async Task<IActionResult> Post([FromBody] ForgotPasswordRequest request)
        {
            ...
            catch (HttpRequestException httpEx) when (httpEx.StatusCode == HttpStatusCode.Unauthorized)
            {
                ...
                return StatusCode(StatusCodes.Status500InternalServerError, new ForgotPasswordResponse
                {
                    Success = false,
            ...
            catch (HttpRequestException httpEx) when (httpEx.StatusCode == HttpStatusCode.UnprocessableContent)
            {
            ...
            catch (HttpRequestException httpEx) when (httpEx.StatusCode == HttpStatusCode.TooManyRequests)
            {
                ...
                return StatusCode(StatusCodes.Status429TooManyRequests, new ForgotPasswordResponse
                {
                    Success = false,
            ...
            catch (ODataError ex)
            {
                ...
                return StatusCode(statusCode: StatusCodes.Status500InternalServerError, new ForgotPasswordResponse
                {
                    Success = false,
